...
class mem_update(nn.Module):
    def __init__(self, act=False):
        ...
        self.act = act
        # 引入 MLF 的三个不同阈值
        self.qtrick1 = MultiSpike_MLF(vth=0.6)
        self.qtrick2 = MultiSpike_MLF(vth=1.6)
        self.qtrick3 = MultiSpike_MLF(vth=2.6)

    def forward(self, x):
        # 为三个层级分别初始化脉冲
        spike1 = torch.zeros_like(x[0]).to(x.device)
        spike2 = torch.zeros_like(x[0]).to(x.device)
        spike3 = torch.zeros_like(x[0]).to(x.device)
        
        output = torch.zeros_like(x)
        mem1_old, mem2_old, mem3_old = 0, 0, 0
        time_window = x.shape[0]
        
        # 注意：这里假设 decay 是在你的全局环境或父类中定义的，与你的原代码保持一致
        for i in range(time_window):
            if i >= 1:
                # 借鉴 MLF 的软重置 (Soft Reset) 思想：发放后膜电位置零
                # 将 (mem_old - spike) 改为 mem_old * (1 - spike)，避免不同阈值下减法重置导致的负值崩塌
                mem1 = mem1_old * (1 - spike1.detach()) * decay + x[i]
                mem2 = mem2_old * (1 - spike2.detach()) * decay + x[i]
                mem3 = mem3_old * (1 - spike3.detach()) * decay + x[i]
            else:
                mem1 = x[i]
                mem2 = x[i]
                mem3 = x[i]
            ...
            # 分别判断是否达到各自的阈值
            spike1 = self.qtrick1(mem1)
            spike2 = self.qtrick2(mem2)
            spike3 = self.qtrick3(mem3)
            ...
            # MLF 核心：总脉冲是各个层级脉冲的总和
            output[i] = spike1 + spike2 + spike3
            
        return output

class MultiSpike_MLF(nn.Module):
    """
    修改了原有的 MultiSpike1，使其支持自定义阈值 (vth)。
    """
    def __init__(self, vth):
        super(MultiSpike_MLF, self).__init__()
        self.vth = vth

    class quant_mlf(torch.autograd.Function):
        @staticmethod
        def forward(ctx, input, vth):
            ctx.save_for_backward(input, torch.tensor(vth))
            # 达到对应的阈值即发放脉冲
        ...
        def backward(ctx, grad_output):
            ...
            v = vth.item()
            # 保持 ori.py 原本的梯度截断逻辑（原版是在 [0, 1] 也就是阈值 0.5 的前后 0.5 范围内传导梯度）
            # 现在平移到对应的阈值附近
            grad_input[input < v - 0.5] = 0
            grad_input[input > v + 0.5] = 0
            return grad_input, None

    def forward(self, x):
        return self.quant_mlf.apply(x, self.vth)
